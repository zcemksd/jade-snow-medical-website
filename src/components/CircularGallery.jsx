...
class Media {
  ...
  // Store image path for click events
...
class App {
  ...
  if (!this.hasMoved && this.onImageClick) {
    const clickedImage = this.getClickedImage(e);
    if (clickedImage) {
  ...
  getClickedImage(e) {
    ...
    // Find which media plane is closest to this X position
    let closestMedia = null;
    let closestDistance = Infinity;
    
    this.medias.forEach(media => {
        const mediaX = media.plane.position.x;
        const distance = Math.abs(mediaX - viewportX);
        
        // Only consider images that are reasonably close (within their width)
        if (distance < media.plane.scale.x / 2 && distance < closestDistance) {
        closestDistance = distance;
        closestMedia = media;
        }
    });
    
    return closestMedia?.imagePath || null;
  }
  ...
  addEventListeners() {
    ...
    // Container-specific events (not global)
